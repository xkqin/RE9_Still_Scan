...
## Published Trajectory Bundles

The repository also stores finalized, UI-loadable camera trajectories generated
by the offline planning pipeline. The planner runs on scored still-image
metadata outside this repository; this repository contains the portable replay
outputs and their diagnostics for collection and review.

### Data flow

```text
scan poses -> capture stills -> EZCAM/OpenCLIP aesthetic scores
           -> select real scored keyframes
           -> plan low-to-high camera routes
           -> validate geometry and score ordering
           -> replay through the UI
```

The exported routes are low-to-high: the first keyframe is a lower-scoring
measured pose and the last keyframe is the selected high-score target. Every
exported planning keyframe is an exact pose from the scored still-image source.
The planner does not fill the JSON with synthetic intermediate scores. Runtime
interpolation may still create unscored video frames between controls.

The current large bundles are:

| Scene | Bundle | Main purpose |
| --- | --- | --- |
| `scene_3.1` | [`15,000 trajectories`](data/trajectory_exports/scene_3.1/trajectories_true_keyframes_gain2p5_distance10_step8_singlemax_globaloverlap90_fast64_15000/) | Outdoor scene, single global maximum endpoint |
| `scene_3.2` | [`13,000 trajectories`](data/trajectory_exports/scene_3.2/trajectories_true_keyframes_gain2p5_distance10_step8_singlemax_globaloverlap90_fast64_13000/) | Hull scene, single global maximum endpoint |
| `scene_2_again` | [`13,000 trajectories`](data/trajectory_exports/scene_2_again/trajectories_true_keyframes_gain2_distance4_step4_singleanchor_balanced_fast64_13000/) | Reprocessed scene-2 capture bundle |
| `scene_1.2` | [`10,000 trajectories`](data/trajectory_exports/scene_1_2_true_keyframes_gain2_distance10_10000/) | Multi-anchor true-keyframe benchmark set |
| `scene_1.3` | [`3,000 trajectories`](data/trajectory_exports/scene_1_3_true_keyframes_gain2_distance10_3000/) | Single-anchor true-keyframe benchmark set |

### What each bundle contains

- `*_low_to_high_ui.json`: the file consumed by the trajectory replay UI.
- `*_trajectories.json`: the full planning representation with measured
  keyframe metadata.
- `trajectory_summary.csv`: one row per trajectory with gain, physical
  distance, path length, duration, and constraint statistics.
- `validation.json` and `capture_readiness_validation.json`: keyframe,
  monotonicity, endpoint, motion, and UI-loader checks.
- `global_route_overlap_audit.json` and
  `global_selection_diagnostics.json`: physical-route uniqueness and
  cross-trajectory overlap diagnostics for the large scene-3 bundles.
- PNG files: score curves, cumulative physical distance, XZ projections, and
  XYZ route views for visual inspection.

### Constraint semantics

The large scene-3 bundles use measured score gain, minimum net displacement,
bounded path length, bounded path-to-net ratio, no near revisit, unique
physical route geometry, and a global route-overlap ceiling. Reusing an
endpoint or a starting region is allowed when the complete physical route
remains unique. The exact thresholds for each bundle are recorded in its JSON
diagnostics rather than inferred from filenames.

These files are planning and replay inputs, not proof that every rendered video
frame is monotonic. A final collection should first run a small smoke capture,
rescore the rendered frames, and check collision behavior. Continuous collision
clearance is not established by still-image poses alone.
