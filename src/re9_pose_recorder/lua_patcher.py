...
def build_lua_block(config: AppConfig, newline: str = "\n") -> str:
    ...
    lines = [
        ...
        "end",
        "",
        "local function re9_pose_create_filter_info()",
        ...
        "        pcall(function() filter:call(\"set_MaskBits\", 0xffffffff) end)",
        "        pcall(function() filter:call(\"set_MaskBits(System.UInt32)\", 0xffffffff) end)",
        "    end",
        "    return filter",
        "end",
        "",
        "local function re9_pose_run_closest_sphere(center, radius)",
        "    if sdk == nil then return false, \"sdk unavailable\", 0 end",
        "    local physics_type = sdk.find_type_definition(\"via.physics.System\")",
        "    if physics_type == nil then return false, \"via.physics.System type not found\", 0 end",
        ...
        "    local filter = re9_pose_create_filter_info()",
        ...
        "    local ok_sphere, sphere_msg, sphere_contacts = re9_pose_run_closest_sphere(origin, sphere_radius)",
        ...
        "    local ok_f, msg_f, count_f, dist_f = re9_pose_run_single_cast_ray(origin, re9_pose_vec3(x + fx * ray_len, y + fy * ray_len, z + fz * ray_len), true)",
        "    rays = rays + 1",
        "    if ok_f then",
        ...
        "    end",
        "    table.insert(detail_parts, \"sphere_r=\" .. string.format(\"%.3f\", sphere_radius) .. \" sphere_contacts=\" .. tostring(sphere_contacts or \"?\") .. \" sphere_ok=\" .. tostring(ok_sphere))",
        ...
        "    re9_pose_logger.physics_probe_contacts = inside_contacts + (tonumber(sphere_contacts) or 0)",
        "    re9_pose_logger.physics_probe_rays = rays",
        "    re9_pose_logger.physics_probe_details = table.concat(detail_parts, \"; \")",
        "    local sphere_suspect = ok_sphere and (tonumber(sphere_contacts) or 0) > 0",
        "    local ray_suspect = near_zero_dirs >= 2 or (min_probe_distance ~= nil and min_probe_distance <= clip_distance)",
        "    local suspect = sphere_suspect or ray_suspect",
        "    re9_pose_logger.physics_probe_pose_valid = not suspect",
        "    if #errors > 0 and not ok_sphere then",
        "        re9_pose_logger.physics_probe_status = \"error\"",
        "        re9_pose_logger.physics_probe_error = table.concat(errors, \" | \")",
        "    elseif suspect then",
        "        re9_pose_logger.physics_probe_status = sphere_suspect and \"sphere_overlap\" or \"suspect_clip\"",
        "        re9_pose_logger.physics_probe_error = \"sphere_contacts=\" .. tostring(sphere_contacts or \"?\") .. \" near_zero_dirs=\" .. tostring(near_zero_dirs) .. \" min_distance=\" .. tostring(min_probe_distance or \"?\") .. \" clip_distance=\" .. tostring(clip_distance)",
