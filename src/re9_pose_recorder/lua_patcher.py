...
def build_lua_block(config: AppConfig, newline: str = "\n") -> str:
    ...
    lines = [
        ...
        "            f:write(string.format('{\"session_id\":\"%s\",\"logging\":%s,\"rows_written\":%d,\"pose_log_file\":\"%s\",\"scan_pose_enabled\":%s,\"scan_segment_id\":\"%s\",\"physics_probe_status\":\"%s\",\"physics_probe_contacts\":%d,\"physics_probe_rays\":%d,\"physics_probe_error\":\"%s\",\"last_error\":\"%s\"}',",
        ...
        "end",
        "",
        "local function re9_pose_run_single_cast_ray(origin, target)",
        ...
        "    re9_pose_call_query(query, \"enableInsideHits\")",
        ...
        "    return true, \"ok\", tonumber(num) or 0",
        "end",
        "",
        ...
        "    local d = 0.35",
        "    local targets = {",
        "        re9_pose_vec3(x + d, y, z), re9_pose_vec3(x - d, y, z),",
        "        re9_pose_vec3(x, y + d, z), re9_pose_vec3(x, y - d, z),",
        "        re9_pose_vec3(x, y, z + d), re9_pose_vec3(x, y, z - d)",
        "    }",
        "    local contacts = 0",
        "    local rays = 0",
        "    local errors = {}",
        "    for _, target in ipairs(targets) do",
        "        local ok, msg, count = re9_pose_run_single_cast_ray(origin, target)",
        "        rays = rays + 1",
        "        if ok then contacts = contacts + (tonumber(count) or 0) else table.insert(errors, tostring(msg)) end",
        "    end",
        "    re9_pose_logger.physics_probe_contacts = contacts",
        ...
        "    else",
        "        re9_pose_logger.physics_probe_status = \"ok\"",
        ...
        "        imgui.text(\"Physics probe: \" .. tostring(re9_pose_logger.physics_probe_status) .. \" contacts=\" .. tostring(re9_pose_logger.physics_probe_contacts) .. \"/\" .. tostring(re9_pose_logger.physics_probe_rays))",
