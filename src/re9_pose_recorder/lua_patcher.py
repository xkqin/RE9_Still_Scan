...
def build_lua_block(config: AppConfig, newline: str = "\n") -> str:
    ...
    lines = [
        ...
        "    if type(freecam_yaw) ~= \"nil\" and type(freecam_pitch) ~= \"nil\" and user_has_rotated then",
        ...
        "    elseif type(captured_matrix) ~= \"nil\" and captured_matrix ~= nil then",
        ...
        "    end",
        "    local targets = {",
        "        {\"forward\", re9_pose_vec3(x + fwd_x * d, y + fwd_y * d, z + fwd_z * d)},",
        "        {\"back\", re9_pose_vec3(x - fwd_x * d, y - fwd_y * d, z - fwd_z * d)},",
        "        {\"right\", re9_pose_vec3(x + right_x * d, y + right_y * d, z + right_z * d)},",
        "        {\"left\", re9_pose_vec3(x - right_x * d, y - right_y * d, z - right_z * d)},",
        "        {\"up\", re9_pose_vec3(x, y + d, z)},",
        "        {\"down\", re9_pose_vec3(x, y - d, z)}",
        "    }",
        "    local contacts = 0",
        "    local inside_contacts = 0",
        "    local min_probe_distance = nil",
        "    local forward_distance = nil",
        ...
        "        if label == \"forward\" then forward_distance = best_dist end",
        ...
        "    local suspect = near_zero_dirs >= 2 or (forward_distance ~= nil and forward_distance <= 0.04)",
        ...
        "        re9_pose_logger.physics_probe_error = \"near_zero_dirs=\" .. tostring(near_zero_dirs) .. \" min_distance=\" .. tostring(min_probe_distance or \"?\") .. \" forward_distance=\" .. tostring(forward_distance or \"?\") .. \" inside_contacts=\" .. tostring(inside_contacts)",
        "    elseif (forward_distance ~= nil and forward_distance < 0.12) or (min_probe_distance ~= nil and min_probe_distance < 0.08) then",
        "        re9_pose_logger.physics_probe_status = \"near_surface\"",
        "        re9_pose_logger.physics_probe_error = \"min_distance=\" .. tostring(min_probe_distance) .. \" forward_distance=\" .. tostring(forward_distance or \"?\") .. \" inside_contacts=\" .. tostring(inside_contacts)",
        "    else",
