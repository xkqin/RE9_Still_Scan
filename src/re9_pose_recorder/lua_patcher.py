...
def build_lua_block(config: AppConfig, newline: str = "\n") -> str:
    ...
    lines = [
        ...
        "            f:write(string.format('{\"session_id\":\"%s\",\"logging\":%s,\"rows_written\":%d,\"pose_log_file\":\"%s\",\"scan_pose_enabled\":%s,\"scan_segment_id\":\"%s\",\"last_error\":\"%s\"}',",
        "                re9_pose_escape_json(payload.session_id), tostring(payload.logging), tonumber(payload.rows_written) or 0,",
        "                re9_pose_escape_json(payload.pose_log_file), tostring(payload.scan_pose_enabled), re9_pose_escape_json(payload.scan_segment_id), re9_pose_escape_json(payload.last_error)))",
        "            f:close()",
        "        end",
        "    end)",
